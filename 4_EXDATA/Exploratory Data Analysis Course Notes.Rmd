...
* calling `plot(x, y)` or `hist(x)` will launch a graphics device and draw a plot on device
	...
	* ***Note**: it is some times necessary to convert column/variable to factor to make plotting easier *
		* `airquality <- transform(airquality, Month = factor(month))`
		with(airquality, plot(Wind, Ozone, col = Month, pch = 20, type = "p", xlab = "wind", ylab = "Ozone"))
...
* **agglomerative approach** (most common) — bottom up 
	...
	* requires
		...
		* **continuous** - *Euclidean distance* --> $\sqrt{(A_1 - A_2)^2 + (B_1 - B_2)^2 + \dots + (Z_1 -Z_2)^2 }$
		* **continuous** - *correlation similarity* --> how correlated two data points are
		* **binary** - *manhattan distance* ("city block distance")--> $|A_1 - A_2| + |B_1 - B_2| + \dots + |Z_1 -Z_2|$
...
* **$U$ and $V$ Matrices**
	- `s <- svd(data)` = performs SVD on data ($n \times m$ matrix) and splits it into $u$, $v$, and $d$ matrices
		+ `s$u` = $n \times m$ matrix --> horizontal variation
		+ `s$d` = $1 \times m$ vector --> vector of the singular/diagonal values
			* `diag(s$d)` = $m \times m$ diagonal matrix
		+ `s$v` = $m \times m$ matrix --> vertical variation
		+ `s$u %*% diag(s$d) %*% t(s$v)` returns the original data --> $X = UDV^T$
